...
async function seed() {
  ...
  await q("INSERT INTO settings (id,role,weather_api_key,gov_api_key,unit_preference,weather_lat,weather_lng) VALUES (?,?,?,?,?,?,?)", [1,"Farmer","","","Metric",18.5204,73.8567]);
  console.log("Settings seeded.");
  const fields = [
    ["f1","North Meadow",12.5,"Loam",18.5254,73.8587,"[[18.527,73.857],[18.527,73.8604],[18.5238,73.8604],[18.5238,73.857]]"],
    ["f2","East Riverfront",8.2,"Clay Loam",18.5190,73.8710,"[[18.521,73.869],[18.521,73.873],[18.517,73.873],[18.517,73.869]]"],
    ["f3","Hillside Slopes",5.4,"Sandy Loam",18.5080,73.8480,"[[18.510,73.846],[18.510,73.850],[18.506,73.850],[18.506,73.846]]"],
    ["f4","South Plateau",10.8,"Black Cotton",18.5010,73.8650,"[[18.503,73.863],[18.503,73.867],[18.499,73.867],[18.499,73.863]]"],
    ["f5","Western Orchards",6.3,"Red Laterite",18.5300,73.8420,"[[18.532,73.840],[18.532,73.844],[18.528,73.844],[18.528,73.840]]"]
  ];
  await q("INSERT INTO fields (id,name,area,soil_type,lat,lng,coordinates) VALUES ?", [fields]);
  console.log("Fields seeded 5.");
  const histories = [
  ...
  const paArr = [
    ["alert-1","Wheat",2150.00,"above","Maharashtra","Pune","Pune APMC",1,"2026-08-25"],
    ["alert-2","Onion",1500.00,"above","Maharashtra","Nashik","Lasalgaon APMC",0,"2026-08-20"],
    ["alert-3","Cotton",7200.00,"above","Maharashtra","Nanded","Nanded APMC",1,"2026-08-22"],
    ["alert-4","Soybeans",4500.00,"above","Maharashtra","Latur","Latur APMC",0,"2026-08-18"],
    ["alert-5","Rice",2300.00,"below","Maharashtra","Kolhapur","Kolhapur APMC",0,"2026-08-10"]
  ];
  await q("INSERT INTO price_alerts (id,crop,targetPrice,alert_condition,state,district,mandi,isTriggered,dateCreated) VALUES ?", [paArr]);
  console.log("Price alerts seeded 5.");
  ...
  await q("INSERT IGNORE INTO users (username,password_hash,full_name,role) VALUES (?,?,?,?),(?,?,?,?)",
    ["demo",demoHash,"Demo Farmer Maharashtra","Farmer","admin",adminHash,"Admin User","Admin"]);
  console.log("Demo users: demo/demo1234 and admin/admin123");
