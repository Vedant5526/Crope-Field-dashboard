...
class App {
  ...
  async showFieldDetail(fieldId) {
    ...
    const logsContainer = document.getElementById("detail-field-history-logs");
        ...
        historyContainer.innerHTML = fieldCrops.map(crop => {
          ...
          return `
            <li class="flex items-center justify-between p-2 rounded-lg bg-emerald-50 border border-emerald-100 mb-2">
              <div>
                <span class="font-bold text-xs text-emerald-900">${crop.crop_name}</span>
    ...
    // Clear the separate activity container if it exists (since we removed it)
  ...
  switchFieldTab(tabName) {
    ['history', 'logs', 'sensor', 'activity'].forEach(t => {
